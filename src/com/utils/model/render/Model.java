...
public class Model implements Cloneable {
    ...
    public void newHeader(byte[] arg0, int arg1) {
        ...
        if (has_fill_attr) faceRenderType = new byte[numTriangles];
        ...
        for (int tri_x = 0; (numTriangles ^ 0xffffffff) < (tri_x ^ 0xffffffff); tri_x++) {
            ...
            if (has_fill_attr) this.faceRenderType[tri_x] = (byte) stream_72_.readByte();
            if (model_priority == 255) trianglePriorities[tri_x] = (byte) stream_73_.readByte();
            if (has_alpha == 1) faceAlpha[tri_x] = (byte) stream_74_.readByte();
            if (has_triangle_skin_types == 1)
                this.triangleSkinValues[tri_x] = (short) stream_75_.readUnsignedByte();
            if (has_textures == 1) faceTexture[tri_x] = (short) (-1 + stream_76_.readUnsignedShort());
            if (hasTexture != null) {
        ...
        if (has_surface_fx) {
            ...
            if (temp_skinAmount > 0) {
                ...
                for (int face = 0; face < temp_skinAmount; face++) {
                    temp_skin[face] = stream.readUnsignedShort();
                    temp_skin_point[face] = stream.readUnsignedShort();
    ...
    public void oldHeader(boolean arg0, byte[] arg1) {
        ...
        for (int i_50_ = 0; (numTriangles ^ 0xffffffff) < (i_50_ ^ 0xffffffff); i_50_++) {
            ...
            if ((i_22_ ^ 0xffffffff) == -2)
                this.triangleSkinValues[i_50_] = (short) stream_19_.readUnsignedByte();
        }
        maxDepth = -1;
        stream.offset = i_36_;
